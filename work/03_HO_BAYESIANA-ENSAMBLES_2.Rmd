...
modelo_GBOOST <- function (train, test, cp =  0, ms = 20, mb = 1, md = 30) { 
  
    modelo <- xgb.cv( 
				data = dtrain,  
				missing = NA,
				stratified = TRUE,       
				nround= nr, # CAMBIE EL 20 POR LA VARIABLE nr PARA OPTIMIZAR CON BAYES
				nfold = 5,
		    ...
		    eta = 0.3,
 				min_child_weight = 1, 
	 			max_depth = md, # CAMBIE EL 6 POR LA VARIABLE md PARA OPTIMIZAR CON BAYES
		 		alpha = 0, 
				lambda = 0, 
...
# CAMBIE EL modelo_rpart: POR xgb GRADIENT BOOSTING!!!
...
"experimento_rpart <- function (ds, semillas, cp =  0, ms = 20, mb = 1, md = 30) {
  auc <- c()  # CREO LISTA PARA GUARDAR LOS AUC
  for (s in semillas) {
    set.seed(s)
    inTraining <- caret::createDataPartition(ds$clase_binaria, p = 0.70, list = FALSE)
    train  <-  ds[  inTraining, ]
    ...
    auc <- c(auc, r) # LISTA CON VALORES DE AUC PARA CADA SEMILLA
  }
  data.table(mean_auc = mean(auc), sd_auc = sd(auc)) # PROMEDIO TODOS LOS AUC DE LA LISTA
}"

experimento_GBOOST <- function (ds, semillas, cp =  0, ms = 20, mb = 1, md = 30) {
  auc <- c()  # CREO LISTA PARA GUARDAR LOS AUC
  for (s in semillas) {
    set.seed(s)
    inTraining <- caret::createDataPartition(ds$clase_binaria, p = 0.70, list = FALSE)
    ...
    test   <-  ds[ -inTraining, ]
    train_sample <- tomar_muestra(train)
    r <- modelo_GBOOST(train[train_sample,], test,  cp = cp, ms = ms, mb = mb, md = md)
    auc <- c(auc, r) # LISTA CON VALORES DE AUC PARA CADA SEMILLA
  }  # CAMBIAR modelo_rpart: POR xgb GRADIENT BOOSTING!!
...
experimento_GBOOST(ds, semillas)
```
...
#### ****MODIFICADO: ENSANMBRE-BAYES:***
set.seed(semillas[1])
S <- optimumLHS(25,2) # GENERA 25 NUMEROS 0 Y 1 AL AZAR

# la primera columna es para el maxdepth, y la segunda para el minslip
#S[,1] <- floor(26*S[,1]) + 4
#S[,2] <- floor(198*S[,2]) + 2

# la primera columna es para el maxdepth, y la segunda para el nround
S[,1] <- floor(26*S[,1]) + 4
S[,2] <- floor(18*S[,2]) + 2
...
for (e in 1:25) {
  r <- experimento_GBOOST(ds, semillas, nr = S[e,2], md = S[e,1]) #CAMBIE EL ms POR nr
  resultados_random_search <- rbindlist( list(resultados_random_search,
                  data.table(md = S[e,1], nr= S[e,2],r) #CAMBIE EL ms POR nr
  ))
}
...
obj.fun = makeSingleObjectiveFunction(
  name = "max depth",
  fn = function(x) - experimento_GBOOST(ds, semillas, md= as.integer(x))$mean_auc,
  par.set = makeNumericParamSet("maxdepth", lower=1L , upper=  10L), #MODIFIQUE EL ESPACIO BUSQUEDA PARA ADAPTARLO AL BOOSTING: MAXIMO DE PROFUNDIDAD LE PUSE DE 1 A 10 (PORQUE EN BOOSTING SE USAN ARBOLES ENANOS)
...
obj_fun <- function(x) { 
  experimento_GBOOST(ds, semillas, md= x$maxdepth, nr= x$nround)$mean_auc #CAMBIO LA VARIABLE ms= x$minsplit POR nr= x$nround
}
...
obj.fun = makeSingleObjectiveFunction(
  ...
  par.set = makeParamSet(
    makeIntegerParam("maxdepth",  lower = 1L, upper = 10L), #MODIFIQUE EL ESPACIO BUSQUEDA PARA ADAPTARLO AL BOOSTING: MAXIMO DE PROFUNDIDAD LE PUSE DE 1 A 10 (PORQUE EN BOOSTING SE USAN ARBOLES ENANOS)
    makeIntegerParam("nround",  lower=1L , upper=  20L) #MODIFIQUE EL ESPACIO BUSQUEDA PARA ADAPTARLO AL BOOSTING: nround de 1 a 20 ARBOLES EN SERIE
  ),
  has.simple.signature = FALSE
)
...
```{r}
#### ****MODIFICADO: ENSANMBRE-BAYES:***
#ggplot(iter, aes(y=nround,x=maxdepth, color=prop.type)) + geom_point(aes(size = y))
ggplot(iter, aes(y=nround,x=maxdepth, color=prop.type)) + geom_point(aes(size = mean_auc))  # XXXX CAMBIE LA y POR mean_auc EN EL size, PORQUE CREO Q ESTABA MAL XXXX

# MODIFIQUE EL EJE Y Y LE PUSE nround

### OBSERVAR EN EL GRAFICO QUE CIRCULO ES MAS GRANDE (MAYOR AUC) Y SELECCIONAR SUS COORDENADAS COMO PARAMETROS OPTIMOS DE nround y maxdepth !!!! CARGAR ESOS OPTIMOS A XGBOOST Y DESPEGAR EL COHETE CON EL MOTOR X-BOOSTING AL KAGGLE!!!!
