...
#clase_binaria1   1={BAJA+2,BAJA+1}    0={CONTINUA}      # IDEA MIA!!!!!!
...
switch ( Sys.info()[['sysname']],
         Windows = { directory.root  <- "C:/DIEGO_/MASTER_DM/2_CUAT_2021/DM_ECON_FINANZAS" })   #Windows
...
fganancia_logistic_lightgbm   <- function(probs, datos) 
{
  ...
  vpesos   <- getinfo(datos, "weight")   # QUE CARAJO ES ESTO?????
  ...
  VPROBS_CORTE  <<- c(VPROBS_CORTE,  tbl[ which.max( tbl$gan_acum ) , prob ] ) ## TIENE ERROR?????? 
...
EstimarGanancia_lightgbm  <- function( x )
{
  ...
  param_basicos  <- list( objective= "binary",   # PARAMETROS Q NO SE OPTIMIZARAN (LOS Q SE OPTIMIZARAN SON: LEARNING RATE, FEATURE FRACTION, MIN DATA IN LEAF, NUM LEAVES)
                          metric= "custom",
  ...
  param_completo  <- c( param_basicos, param_variable, x ) # "x" DEBERIAN SER LOS PARAMETROS OPTIMIZADOS CON BAYES!!!??? 

  VPROBS_CORTE  <<- c()
  set.seed( 999983 )
  modelocv  <- lgb.cv( data= dtrain,   #OJO: dtrain NO ESTA AUN DEFINIDO XXXX !!!!
                       eval= fganancia_logistic_lightgbm,   # COMO HACE PARA EVALUAR SOBRE UNA LISTA CON 3 COLUMNAS????
  ...
  ganancia  <- unlist(modelocv$record_evals$valid$ganancia$eval)[ modelocv$best_iter ]  # ESTO ES PARA PODER EVALUAR LA LISTA DE 3 COLUMNAS??????
